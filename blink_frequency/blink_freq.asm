;-------------------------------------------------------------------------------
; frequency.asm  (MSP430FR2433)  **FULL WORKING CODE**
;
; Lab requirement:
; - SMCLK/MCLK = 8 MHz
; - UART0 = 9600 baud, 8-N-1, using interrupts (RX + TX)
; - LED P1.1 blinks at user-selected frequency (1 Hz to 20 Hz), 50% duty
...
;-----------------------------
; UART @ 8MHz -> 9600 baud (oversampling)
; (Same ratio as 16MHz->19200: 833.33)
;-----------------------------
U0_UCBR     .equ    52
U0_UCBRF    .equ    1
U0_UCBRS    .equ    0x49
...
;-------------------------------------------------------------------------------
SetupCLK    bis.w   #SCG0, SR                    ; Disable FLL
            mov.w   #SELREF__REFOCLK, &CSCTL3    ; FLL ref = REFO
            clr     &CSCTL0                      ; clear DCO tap/mod

            ; choose a DCO range that supports ~8MHz (DCORSEL_3 is common)
            bic.w   #DCORSEL_7, &CSCTL1
...
;-------------------------------------------------------------------------------
            ...
            ; do NOT set P1.4/P1.5 as GPIO outputs

;-------------------------------------------------------------------------------
; UART setup: eUSCI_A0, SMCLK, 9600, enable RX interrupt
;-------------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------
; TimerA0: ACLK = 32768 Hz, up mode
; Start at 1 Hz: CCR0 = 16384 - 1
;-------------------------------------------------------------------------------
...
; R15 = pointer to null-terminated string
; - waits until idle (simple + reliable for lab)
;-------------------------------------------------------------------------------
StartTX
            push    R14
